...
function AuthPageInner() {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col" style={{ backgroundColor: "#FFF8E8" }}>
  ...
  <div className="w-full max-w-sm flex flex-col items-center justify-center gap-7">
    ...
    >
      <button
        ...
        style={{
          ...
          border: "none",
          cursor: "pointer",
          backgroundColor: tab === "login" ? "#FF7A00" : "transparent",
          color: tab === "login" ? "#fff" : "#2B1B12",
          boxShadow: tab === "login" ? "0 10px 24px rgba(255, 122, 0, 0.22)" : "none",
        }}
      >
        Đăng nhập
      </button>
      <button
        ...
        style={{
          ...
          border: "none",
          cursor: "pointer",
          backgroundColor: tab === "register" ? "#FF7A00" : "transparent",
          color: tab === "register" ? "#fff" : "#2B1B12",
          boxShadow: tab === "register" ? "0 10px 24px rgba(255, 122, 0, 0.22)" : "none",
        }}
      >
        Đăng ký
      </button>
    ...
    <div className="w-full">
      <div className="flex items-center gap-3 my-6">
        <div className="h-px flex-1" style={{ backgroundColor: "#E8D8C3" }} />
        <span className="text-xs" style={{ color: "#7A6A5C", whiteSpace: "nowrap" }}>
          Hoặc đăng nhập với
        </span>
        <div className="h-px flex-1" style={{ backgroundColor: "#E8D8C3" }} />
      </div>
      ...
      <div className="flex justify-center gap-4">
        ...
        <button
          ...
          style={{ borderColor: "#2B1B12" }}
        >
          <span className="font-bold text-sm" style={{ color: "#2B1B12" }}>
            P
          </span>
        </button>
      </div>
...
function Field({ icon, type = "text", placeholder, rightSlot }) {
  return (
    ...
    >
      ...
      />
      {rightSlot ? (
        <span className="flex-shrink-0 flex items-center justify-center">{rightSlot}</span>
      ) : null}
    </div>
  );
}
...
function LoginForm({ showPwd, setShowPwd }) {
  ...
  const validate = () => {
    ...
    if (formData.password.length < 6) newErrors.password = "Mật khẩu phải từ 6 ký tự";
    setErrors(newErrors);
  ...
  return (
    <form className="space-y-7" onSubmit={handleSubmit}>
      <h2 className="text-center text-2xl font-bold mb-8" style={{ color: "#2B1B12" }}>
        Chào mừng trở lại
      </h2>

      {/* Email Field */}
      <div className="space-y-2">
        <label className="text-sm font-semibold tracking-wide block" style={{ color: "#2B1B12" }}>
      ...
      <div className="space-y-2">
        ...
        <Field
          ...
          rightSlot={
            <button type="button" onClick={() => setShowPwd((v) => !v)} className="p-1">
              {showPwd ? <EyeIcon size={16} color="#7A6A5C" /> : <EyeOffIcon size={16} color="#7A6A5C" />}
            </button>
          }
        />
      ...
      <div className="flex items-center justify-between text-xs pt-2">
        <label className="flex items-center gap-2 cursor-pointer font-medium" style={{ color: "#2B1B12" }}>
      ...
      <div className="space-y-5">
        ...
        <InputField 
          ...
          rightSlot={
            ...
            >
              {showPwd ? <EyeIcon size={18} color="#7A6A5C" /> : <EyeOffIcon size={18} color="#7A6A5C" />}
            </button>
          }
        />
      </div>
...
function InputField({ label, icon, error, rightSlot, ...props }) {
  return (
    <div className="flex flex-col gap-2">
      <label className="text-sm font-semibold tracking-wide text-[#2B1B12]">{label}</label>
      <div className={`flex items-center gap-2 border px-3 py-3 rounded-xl bg-white ${error ? 'border-red-500' : 'border-gray-200'} focus-within:ring-2 focus-within:ring-orange-200 transition-all`}>
        {icon}
